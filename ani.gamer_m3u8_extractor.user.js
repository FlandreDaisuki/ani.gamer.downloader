...
const snListNodes = $('.season .playing').parent().find('a').toArray();
...
Promise.all(snList.map(a => a.sn).map(getPlaylistSrc))
	.then(plSrcs => {
...
window.snList = snList;

snList.getResolution = function(p) {
	return this.map(ep => {
		const src = ep.streams[p].src;
		const magic = src.replace(/(^.)*-video.*/, '$1');
		const m3u8 = `https://gamer-cds.cdn.hinet.net/vod/gamer/${magic}/hls-ae-2s/${src}`;
		return {name: ep.name, m3u8};
	});
};
...
for(let res of [1080, 720, 540, 360]) {
	$(`<div class="sn-res">點擊儲存 ${res}p 的 m3u8</div>`)
		...
		.click(() => {
			const blob = new Blob([JSON.stringify(snList.getResolution(`${res}p`), null, 2)], {type: "text/plain;charset=utf-8"});
